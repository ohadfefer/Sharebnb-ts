...
import { buildNext30Occupancy, leadTimeBuckets, revenueByListing } from '../services/analytics.service.js'
...
export function InsightsRow({ orders }: { orders: Order[] }) {
    ...
    const hasAny = (arr: any[]) => Array.isArray(arr) && arr.length && arr.some(x => (x.v ?? x.total ?? x.occ) > 0)
                ...
                {hasAny(lead) ? (
                    <div className="chart-wrap">
                        <ResponsiveContainer width="100%" height={200}>
                            <PieChart>
                                <Pie data={lead as any} dataKey="v" innerRadius={55} outerRadius={80} paddingAngle={2}>
...
function Next30Calendar({ occ = [] }: { occ: any[] }) {
    ...
    const cells = [...leading, ...days] as any[]
