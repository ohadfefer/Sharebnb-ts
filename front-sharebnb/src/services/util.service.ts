...
export function saveToStorage(key: any, value: any): void {
    localStorage.setItem(key, JSON.stringify(value))
}

export function loadFromStorage(key: any): void {
...
type ParamsFilter = Partial<{ address: string, checkIn: string, checkOut: string, guests: any }>

export function buildSearchParams(filter: ParamsFilter) {
...
export function parseSearchParams(searchParams: any): ParamsFilter {
    const params = searchParams instanceof URLSearchParams
        ? searchParams
        : new URLSearchParams(searchParams)

    const filter = {} as ParamsFilter
    const value = (key: any) => params.get(key) || ""
...
/**
 * Calculates number of nights between two ISO dates (hotel-style)
 * Sets time to noon to avoid DST/day-boundary issues
...
/**
 * Converts Date object → YYYY-MM-DD string
...
/**
 * Converts YYYY-MM-DD string → Date object (or undefined)
 */
...
/**
 * Builds stay URL with optional search params
 */
export function buildStayPathWithParams(
    stayId: string | number,
    searchParams?: URLSearchParams | null
): string {
    const query = searchParams?.toString() ?? ''
    return query ? `/stay/${stayId}?${query}` : `/stay/${stayId}`
}

/**
 * Formats date range in short human-readable form:
 * - "8–9 Sept"
 * - "28 Sept – 2 Oct"
