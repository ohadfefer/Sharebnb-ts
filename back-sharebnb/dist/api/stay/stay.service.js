...
async function getById(stayId) {
    try {
        ...
        if (!stay)
            throw new Error(`Stay not found: ${stayId}`); // NEW
...
async function remove(stayId) {
    ...
    try {
        ...
        if (!_id)
            throw new Error(`Invalid stay id: ${stayId}`); // NEW
        const criteria = { _id };
        if (!isAdmin)
...
async function update(stay) {
    ...
    try {
        ...
        if (!_id)
            throw new Error(`Invalid stay id: ${stay?._id}`); // NEW
        const collection = await dbService.getCollection('stay');
...
function _buildCriteria(filterBy) {
    ...
    if (filterBy.address && filterBy.address.trim()) {
        ...
        addressOr.push({ 'loc.city': { $regex: searchTerm, $options: 'i' } }, { 'loc.country': { $regex: searchTerm, $options: 'i' } }, { 'loc.address': { $regex: searchTerm, $options: 'i' } }, { 'name': { $regex: searchTerm, $options: 'i' } });
        ...
        if (searchTerm.includes('fira')) {
            addressOr.push({ 'loc.city': { $regex: 'santorini', $options: 'i' } }, { 'loc.address': { $regex: 'greece', $options: 'i' } });
        }
        if (searchTerm.includes('greece')) {
            addressOr.push({ 'loc.city': { $regex: 'santorini|athens|mykonos|crete', $options: 'i' } }, { 'loc.address': { $regex: 'greece', $options: 'i' } });
        }
...
async function addToWishlist(stayId, userId) {
    try {
        ...
        if (!_id)
            throw new Error(`Invalid stay id: ${stayId}`);
        const collection = await dbService.getCollection('stay');
        ...
        await collection.updateOne({ _id }, { $push: { wishlist: wishlistEntry } });
        // Return updated stay
...
async function removeFromWishlist(stayId, userId) {
    try {
        ...
        if (!_id)
            throw new Error(`Invalid stay id: ${stayId}`);
        const collection = await dbService.getCollection('stay');
        await collection.updateOne({ _id }, { $pull: { wishlist: { userId } } });
        // Return updated stay
        const updatedStay = await collection.findOne({ _id });
        return updatedStay;
    }
